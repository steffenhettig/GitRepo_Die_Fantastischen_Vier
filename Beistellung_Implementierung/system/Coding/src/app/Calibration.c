   ...
   if(!firstTimeFlag)
   {
    ...
    case CALIBRATION_STATE_CENTER_ON_LINE:
       ...
       if (SOFTTIMER_IS_EXPIRED(&gTimer))
       {
        ...
        && ((CALIB_BETWEEN_LINE_DOWN_THRESHOLD < sensorValues.value[LINESENSOR_MIDDLE_LEFT]) 
        && (CALIB_BETWEEN_LINE_UP_THRESHOLD > sensorValues.value[LINESENSOR_MIDDLE_LEFT]))
        && (CALIB_OVER_LINE_THRESHOLD < sensorValues.value[LINESENSOR_MIDDLE])
        && ((CALIB_BETWEEN_LINE_DOWN_THRESHOLD < sensorValues.value[LINESENSOR_MIDDLE_RIGHT]) 
        && (CALIB_BETWEEN_LINE_UP_THRESHOLD > sensorValues.value[LINESENSOR_MIDDLE_RIGHT]))
